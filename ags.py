...
if __name__ == '__main__':
    ...
    limit = 3 #3 metros cubicos
    
    #Trabalhando e iniciando o algoritmo genetico
    tamanho_populacao = 20
    ag = algoritimoGenetico(tamanho_populacao)
    ag.inicializa_populacao(espacos,valores,limit)#iniciar a populacao
    
    #iniciar avalição da população e ordenar
    for individuo in ag.populacao:
        individuo.avaliacao()
    ag.ordenar_populacao()                        #ordenar a populacap
    ag.melhor_individuo(ag.populacao[0])
    soma = ag.soma_avaliacoes()#avaliação da populacao total
    
    #--------------------------------------------------------------------
    
    #Gerar nova populacao e aplicar operadores geneticos
    nova_populacao = []
    probabilidade_mutacao = 0.01;
    #Selecionar pais usando metodo da roleta viciada
    for individuos_gerados in range(0,ag.tamanho_populacao,2):
        pai1 = ag.selecionar_pai(soma)
        pai2 = ag.selecionar_pai(soma)
        
        filho = ag.populacao[pai1].crossover(ag.populacao[pai2])
        nova_populacao.append(filho[0].mutacao(probabilidade_mutacao))
        nova_populacao.append(filho[1].mutacao(probabilidade_mutacao))
    ...
    for i in range(tamanho_populacao):
      print("\n#Individuo %s° \n\n" % i,
            "\nEspaco     %s" % str(ag.populacao[i].espacos),
            "\nValores    %s" % str(ag.populacao[i].valores),
            "\nCromossomo %s" % str(ag.populacao[i].cromossomo),
            "\nNota %s"       % str(ag.populacao[i].nota_avaliacao))                            
    '''
   
 
    '''
    individuo1 = Individuo(espacos,valores,limit)
    print("individuo-1")
    for i  in range(len(lista)):
      if (individuo1.cromossomo[i] == '1'):
        print("nome: %s RS= %s" % (lista[i].nome,lista[i].valor))
                
    individuo1.avaliacao()
    print("Nota= %s"   % individuo1.nota_avaliacao)
    print("Espaço: %s" % individuo1.espaco_usado)
        
   
    individuo2 = Individuo(espacos,valores,limit)
    print("\n individuo-2")
    for i  in range(len(lista)):
    ...
    print("Espaço: %s" % individuo2.espaco_usado)
    
    individuo1.crossover(individuo2)
    
    #Mutação no gens do individuo
